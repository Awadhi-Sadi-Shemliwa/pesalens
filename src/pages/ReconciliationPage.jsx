...
const STATUS_TONE = {
  fully_explained: { label: 'Fully explained', tone: 'inc' },
  partial:         { label: 'Partial',         tone: 'exp' },
  blind_spot:      { label: 'Blind spot',      tone: 'dng' },
};
      ...
      <div className="max-w-6xl mx-auto space-y-5">
        ...
        {kpis && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <KpiTile label="Explained"      value={fmtTZSFull(kpis.total_explained)} icon="check"   tone="inc" />
            <KpiTile label="Blind spot"     value={`${blindPct}%`}                    icon="alert"   tone={blindPct >= 50 ? 'dng' : 'exp'} />
            <KpiTile label="Tracked events" value={String(kpis.group_count)}          icon="chart" />
          </div>
        )}
        ...
        {/* Group table -------------------------------------------------- */}
        <div className="bg-surface-2 border border-bdr rounded-2xl overflow-hidden">
          <div className="px-4 py-3 border-b border-bdr flex items-center justify-between">
            <Eyebrow>From statement → potential usage</Eyebrow>
...
const KpiTile = ({ label, value, icon, tone }) => {
  const toneClass = tone === 'inc' ? 'text-inc'
                  ...
                  :                   'text-txt-1';
  return (
    <div className="bg-surface-2 border border-bdr rounded-2xl p-3.5">
      <div className="flex items-center justify-between text-[11px] text-txt-3 font-mono uppercase tracking-ticker">
        <span>{label}</span>
        <Icon name={icon} size={12} />
      </div>
      <div className={`mt-2 text-lg sm:text-xl font-bold tabular ${toneClass}`}>{value}</div>
    </div>
  );
};
  ...
  return (
    <li>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full px-4 py-3 flex items-center gap-3 text-left hover:bg-surface-3/40"
      >
        <div className="text-[11px] font-mono uppercase tracking-ticker text-txt-3 w-24 flex-shrink-0">
          {dateLabel}
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-[13px] font-semibold text-txt-1 truncate">
            {KIND_LABEL[g.kind] || 'Debit'} · {fmtTZS(g.debit_amount)}
          </div>
...
const PatternsPanel = ({ patterns }) => {
  ...
  return (
    <div className="grid md:grid-cols-3 gap-3">
      <div className="bg-surface-2 border border-bdr rounded-2xl p-4">
