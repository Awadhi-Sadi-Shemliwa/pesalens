/* Password strength — entropy, not rule-checkboxes (password-field.md #1–2).
   ...
   PesaLens-MobileAPP/src/data/password.ts. Keep the two in sync. */
...
export function passwordRulesMet(pw = '') {
...
/**
 * @returns {{ bits:number, score:0|1|2|3, word:string, reasons:string[] }}
 *   `score` indexes a 4-segment meter; `word` is the rating that must accompany
 *   the colour so the signal survives colour-blindness (password-field.md #5).
...
export function passwordStrength(pw = '') {
  ...
  let score = bits < 28 ? 0 : bits < 40 ? 1 : bits < 60 ? 2 : 3;
